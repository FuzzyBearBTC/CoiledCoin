...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 0) {
        ...
        nSubsidy >>= (nHeight / 525000);
        // Unlike Bitcoin, we stop at a subsidy of 1.0
        if(nSubsidy < 1 * COIN)
            nSubsidy = 1 * COIN;
    }
    ...
    if (hashBestChain == hash)
        CRITICAL_BLOCK(cs_vNodes)
            BOOST_FOREACH(CNode* pnode, vNodes)
                if (nBestHeight > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : 0 /* FIXME - add block estimate here */))
...
unsigned char pchMessageStart[4] = { 0x92, 0xd5, 0xa8, 0x8c };
